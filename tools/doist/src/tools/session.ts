...
import { filterByEnergy } from "../analysis/index.ts";
...
export function registerSessionTools(mcp: McpServer, container: Container): void {
	registerTool({
		mcp,
		...
		spanOptions: ({ energy }) => ({ attributes: { energy: energy ?? "none" } }),
		callback: async ({ energy, sync: shouldSync }) => {
			const { db, client, listProjectIds } = container;
			const sync = await maybeSyncSummary(db, client, listProjectIds, shouldSync);
			...
			const suggested = energy ? filterByEnergy(db.selectTasks(), energy).map(enrich) : [];
			const syncedAt = db.getLastSyncedAt();

			return {
				data: { sync, overdue, today, thoughtsCount: thoughts.length, requiresTriage, suggested, syncedAt },
