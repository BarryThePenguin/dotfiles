...
import { recordException, tracer, trackOperation } from "doist-core";

function propagateContext({ mcpReq }: ServerContext): api.Context {
	return api.propagation.extract(api.context.active(), mcpReq._meta);
}

type ToolResult<Schema extends StandardSchemaWithJSON> = {
