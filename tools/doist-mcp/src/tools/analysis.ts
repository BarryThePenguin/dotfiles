...
export function registerAnalysisTools(
	...
	registerTool({
		...
		callback: async ({ sync: shouldSync }) => {
			...
			const staleByProject = groupStaleByProject(
				enrichedStaleCandidates,
				projects,
			);
			...
			const unroutedInbox = inboxId
				? db
						...
						.map(enrich)
				: [];
			const missingEnergyMetadata =
				findMissingEnergyMetadata(allTasks).map(enrich);
			...
			return {
				data: {
					...
					stale: {
						...stale,
						candidates: enrichedStaleCandidates,
						byProject: staleByProject,
					},
					unroutedInbox,
					missingEnergyMetadata,
					requiresAttention,
